...
const startSwappers = async (lpTok) =>{
    ...
    const ctc = acc.contract(backend, ctcDeployer.getInfo());
            ...
            console.log(`${swapper} current bal of tokA is ${await acc.balanceOf(tokA.id)}`)
            console.log(`${swapper} current bal of tokB is ${await acc.balanceOf(tokB.id)}`)

        try {
            // depositing tokens to liquidity pool
            const res = await ctc.apis.Swapper.deposit({firstTok: tokA, secondTok: tokB, firstSupply:10, secondSupply:20})
            console.log(res)
            console.log(`${swapper} post bal of tokA is ${await acc.balanceOf(tokA.id)}`)
            console.log(`${swapper} post bal of tokB is ${await acc.balanceOf(tokB.id)}`)
